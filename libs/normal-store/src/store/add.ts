import type { GetKey, Normalized, NormalizedValue } from '@/types';
import { appendUnique, prependUnique } from '@/utils';

import type { AddMany } from './addMany';
import { appendMany, prependMany } from './addMany';
...
export const prependOne: AddOne = <T extends Normalized<any>>(
  normalized: T,
  key: string,
  value: NormalizedValue<T>
): T => ({
  ...updateOne(normalized, key, value),
  allKeys: prependUnique(normalized.allKeys, key),
});

export const appendOne: AddOne = <T extends Normalized<any>>(
  normalized: T,
  key: string,
  value: NormalizedValue<T>
): T => ({
