...
describe('store | patch', () => {
  describe('patchOne()', () => {
    ...
    it('patches an existing value by transforming the current value', () => {
      expect(patchOne(store, 'a', ({ counter }) => ({ counter: counter + 2 }))).to.eql({
        allKeys: ['a'],
  ...
  describe('patchMany()', () => {
    ...
    it('patches an existing values', () => {
      expect(patchMany(store, [{ key: 'a', value: { counter: 1 } }])).to.eql({
        allKeys: ['a'],
        byKey: { a: { id: 'a', counter: 1 } },
      });
    });
  ...
  describe('patch()', () => {
    ...
    it('patches one existing value by a key', () => {
      expect(patch(store, 'a', { counter: 1 })).to.eql({
        allKeys: ['a', 'b'],
        byKey: { a: { id: 'a', counter: 1 }, b: { id: 'b', counter: 2 } },
      });
    });
