import type { Normalized, NormalizedValue } from '@/types';
...
type PatchValue<T extends Normalized<any>> =
  | Partial<NormalizedValue<T>>
  | ((value: NormalizedValue<T>) => Partial<NormalizedValue<T>>);
...
export const patch: Patch = <T extends Normalized<any>>(
  store: T,
  keyOrPatches: string | PatchManyItem<T>[],
  value?: PatchValue<T>
): T =>
