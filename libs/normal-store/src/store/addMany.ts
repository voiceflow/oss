import type { GetKey, Identifiable, Normalized, NormalizedValue } from '@/types';
...
export const prependMany: AddMany = <T extends Normalized<any>>(
  store: T,
  values: NormalizedValue<T>[],
  getKey?: GetKey<NormalizedValue<T>>
) => addMany(store, values, (allKeys, newKeys) => [...newKeys, ...allKeys], getKey);

export const appendMany: AddMany = <T extends Normalized<any>>(
  store: T,
  values: NormalizedValue<T>[],
  getKey?: GetKey<NormalizedValue<T>>
