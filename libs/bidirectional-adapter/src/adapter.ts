...
type RemapFromDBKeys<
  DBModel,
  Model,
  PartialDBModel extends Partial<DBModel>,
  KeyRemap extends [keyof PartialDBModel, keyof Model][],
> = Pick<Model, Extract<keyof Model, Exclude<keyof PartialDBModel, KeyRemap[number][0]>>> &
...
type RemapToDBUnion<
  ...
  KeyRemap extends [keyof DBModel, keyof PartialModel],
> = KeyRemap extends [infer ToDBKey extends keyof DBModel, infer FromDBKey extends keyof PartialModel]
...
type RemapToDBKeys<
  ...
  PartialModel extends Partial<Model>,
  KeyRemap extends [keyof DBModel, keyof PartialModel][],
> = Pick<DBModel, Extract<keyof DBModel, Exclude<keyof PartialModel, KeyRemap[number][1]>>> &
...
> {
  ...
  <PartialDBModel extends Partial<DBModel>>(
    dbModel: PartialDBModel,
    ...args: FromDBArgs
  ): RemapFromDBKeys<DBModel, Model, PartialDBModel, KeyRemap>;
}
...
export interface SmartMapFromDB<
  DBModel,
  Model,
  FromDBArgs extends any[] = [],
  KeyRemap extends [keyof DBModel, keyof Model][] = [],
...
> {
  ...
  <PartialModel extends Partial<Model>>(
    model: PartialModel,
    ...args: ToDBArgs
  ): RemapToDBKeys<DBModel, Model, PartialModel, KeyRemap>;
}
